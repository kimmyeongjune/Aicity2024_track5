...
ap.add_argument("-t", "--gt_txt", help="path to gt txt", default="./data/aicity/aicity2024_track5_train/gt.txt")
ap.add_argument("-v", "--input_video_dir", help="path to input video dir", default="./data/aicity/aicity2024_track5_train/videos")
ap.add_argument("-o", "--output_video_dir", help="path to output video dir", default="./data/aicity/crop_data/crop_images")
ap.add_argument("-p", "--part_gt_dir", help="path to part gt dir", default="./data/aicity/crop_data/crop_labels")
...
def convert(line,imgWidth,imgHeight):
    left, top, width, height, classes = np.array(line[1:],float)
    x = (left + width / 2.0) / imgWidth
    y = (top + height / 2.0) / imgHeight
    w = width / imgWidth
    h = height / imgHeight
    classes = classes-1
    return np.array([classes, '%.6f'%x, '%.6f'%y, '%.6f'%w, '%.6f'%h],float) # 保留6位小数
